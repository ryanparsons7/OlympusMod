class Enh_AmbientAnimation: Title
{
	...
	{\
		if ((_value select 1) == getText (_x >> 'data')) then\
		{\
	...
	attributeSave =
	"\
	private _animCtrl = _this controlsGroupCtrl 100;\
	private _gearCtrl = _this controlsGroupCtrl 101;\
	_value = [_animCtrl lbData lbCurSel _animCtrl,_gearCtrl lbData lbCurSel _gearCtrl];\
	_value";

	h = ATTRIBUTE_CONTENT_W * GRID_W / 3;
	class Controls: Controls
	{
		class AnimText: ctrlStructuredText
		{
			idc = -1;
			x = 5 * GRID_W;
		...
		class Reset: ctrlButton
		{
			...
			onButtonDown  = "((ctrlParentControlsGroup (_this select 0)) controlsGroupCtrl 100) lbSetCurSel 0; ((ctrlParentControlsGroup (_this select 0)) controlsGroupCtrl 101) lbSetCurSel 0";
