...
private _getColorFromHex =
{
    ...
    private _hex = toArray (_this select 0);
    private _hex = _hex - [(_hex select 0)];
...
sleep 0.005;
//Close display by pressing CTRL + B && save exit with ESC
(findDisplay 50000) displayAddEventHandler
[
   "keyDown",
   {
      if (((_this select 1) == 48 && (_this select 3)) || (_this select 1 == 1)) then
      {
...
GET_CONTROL(50000,30) ctrlSetText (_historyLatest select 0);//Briefing Title
GET_CONTROL(50000,20) ctrlSetText (_historyLatest select 1);//Subject Text
GET_CONTROL(50000,10) ctrlSetText (_historyLatest select 2);//Briefing Text
...
{
   private _briefingTitle = _x select 0;
   private _briefingText = _x select 1;
   //private _briefingSubject = (_templates select _forEachIndex) select 2; Currently not used

   GET_CONTROL(50000,80) lbAdd _briefingTitle;
   GET_CONTROL(50000,80) lbSetData [_forEachIndex,_briefingText];
} forEach _templates;
